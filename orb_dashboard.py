...
Run:
    streamlit run orb_dashboard.py
"""
...
# ── Dashboard scan (runs scan directly, no orb_main.py required) ───────────────
...
def _run_dashboard_scan() -> tuple[str, str]:
    ...
    return "success", (
        ...
        f"ranked by Relative Volume. Table updated below."
    )
...
# ── Controls: Start/Stop Trading + Run Scan ────────────────────────────────────
...
_pre_watchlist = _t < dtime(8, 0)        # before 8 AM — too early for watchlist rebuild
_pre_open      = dtime(8, 0) <= _t < dtime(9, 30)  # 8:00–9:29 — ideal start window
...
# Flash message — shown once after a button action, then cleared
if "_flash" in st.session_state:
    flash = st.session_state.pop("_flash")
    _display = {"success": st.success, "warning": st.warning, "error": st.error, "info": st.info}
    _display.get(flash["level"], st.info)(flash["msg"])
