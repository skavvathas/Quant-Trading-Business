"""
run_orb_tqqq_backtest.py — Entry point for the ORB TQQQ backtest.

Based on: Zarattini & Aziz, "Can Day Trading Really Be Profitable?" (2023/2025)
Instrument: TQQQ (ProShares UltraPro QQQ — 3× leveraged Nasdaq-100 ETF)

TQQQ advantage: its lower share price lets a $25k account access the same
dollar daily exposure as a $300k QQQ account, bypassing the 4× leverage cap
that caps most retail ORB QQQ returns. Paper result: 1,484% total return
(2016–2023), Sharpe 1.19, MDD 28%.

Usage:
    python run_orb_tqqq_backtest.py                          # paper period, both variants
    python run_orb_tqqq_backtest.py --start 2020-01-01 --end 2024-12-31
    python run_orb_tqqq_backtest.py --variants optimised     # optimised only
    python run_orb_tqqq_backtest.py --skip-download          # if bars are already cached
    python run_orb_tqqq_backtest.py --download-only          # fetch bars, skip simulation
"""
...
def main():
    ...
    parser.add_argument("--tag",           default="",
                        help="Output file suffix to avoid overwriting other periods "
                             "(e.g. --tag oos → orbqqq_tqqq_baseline_oos_report.csv)")
